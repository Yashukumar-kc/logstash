...
===== ARM64 support for MacOS (experimental) 
...
// **JDK 15 **
// JDK15 changes - new jvm.options file - we might want to flag this, it moves us a jvm.options file
// that looks like the one that comes with Elasticsearch
...
// Fix issue where pipelines in the loading state appear as running, causing errors in the logs until the pipeline finishes loading
...
// Enables a way to allow plugins to trigger pipeline reloads. Relates to geoip work
...
// Fix launching logstash with bundled JDK when folder contains spaces on Windows
...
// Use correct headers API to handle redirects in plugin manager
...
// TODO: Make sure LS exits when running into fatal errors, such as OutOfMemoryErrors, and improve logging
// in these scenarios
* 53e73e9cc - Fix: make sure LS exits when running into fatal errors (#12559) (5 weeks ago) <Karol Bucek>

// TODO: Sumarise PQ corrution bug
* e2f2255fa - pq: eliminate corruption by forcing version byte to be persisted (5 weeks ago) <Ry Biesemeyer>

// TODO: This informs users that certain command line flags have been deprecated - warning was previously hidden
...
// Kinda related to some of the JDK changes - no need to promote in and of itself
...
* 567487fdc - Make integration test fail if 'offline' tool can't be compiled (5 hours ago) <andsel>
